...
def stage2_analysis(uploaded_file):    
   if uploaded_file is not None:  
      ...
      try:  
        # Directly convert to datetime, handling standard date formats  
      ...
      for i, row in df.iterrows():  
        ...
        if sma50 is not None and not sma50.empty and not sma150.empty and not sma200.empty:  
           ...
           if len(close) > 0 and len(sma150) > 0 and len(sma200) > 0:  
              # Criteria checks  
              is_price_above_sma_150_and_200 = (close.iloc[-1] > sma150.iloc[-1]) and (close.iloc[-1] > sma200.iloc[-1])  
              ...
              is_sma_50_above_sma_150_and_200 = (sma50.iloc[-1] > sma150.iloc[-1]) and (sma50.iloc[-1] > sma200.iloc[-1])  
...
# Run the application  
stage2_analysis('holdings_with_corrected_tickers.xlsx')